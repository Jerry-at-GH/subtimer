...
from nfa_utils.constants import BLANK_TOKEN, SPACE_TOKEN
from nfa_utils.data_prep import Segment
...
def make_ctm_files(
    ...
    utt_obj = make_ctm(utt_obj, output_dir_root, audio_file_duration, ctm_file_config)
...
def make_ctm(
    ...
    output_dir = os.path.join(output_dir_root, "ctm", "tokens")
    ...
    with open(os.path.join(output_dir, f"{utt_obj.utt_id}.ctm"), "w") as f_ctm:
        for boundary_info_ in boundary_info_utt:  # loop over every token/segment
            ...
            if not (boundary_info_.t_start < 0 or boundary_info_.t_end < 0):
                ...
                if (
                    ...
                    # make the predicted duration of the token/segment longer, growing it outwards equal
                    # amounts from the predicted center of the token/segment
                    token_mid_point = (start_time + end_time) / 2
                    start_time = max(token_mid_point - ctm_file_config.minimum_timestamp_duration / 2, 0.0)
                    end_time = min(token_mid_point + ctm_file_config.minimum_timestamp_duration / 2, audio_file_duration)

                if not (text == BLANK_TOKEN and ctm_file_config.remove_blank_tokens):  # don't save blanks if we don't want to
    ...
    utt_obj.saved_output_files["tokens_level_ctm_filepath"] = os.path.join(output_dir, f"{utt_obj.utt_id}.ctm")
