...
from nfa_utils.constants import BLANK_TOKEN, SPACE_TOKEN, V_NEGATIVE_NUM
...
def restore_token_case(text_str, tokens):
    ...
    while "__" in text_str:
        text_str = text_str.replace("__", "_")
    ...
    for token in tokens:
        ...
        for token_char in token:
            if token_char == text_str[text_char_pointer]:
                token_cased += token_char
                text_char_pointer += 1

            else:
                if token_char.upper() == text_str[text_char_pointer] or is_sub_or_superscript_pair(
                    token_char, text_str[text_char_pointer]
                ):
                    token_cased += token_char.upper()
                    text_char_pointer += 1
                else:
                    if token_char == "▁" or token_char == "_":
                        if text_str[text_char_pointer] == "▁" or text_str[text_char_pointer] == "_":
                            token_cased += token_char
                            text_char_pointer += 1
                        elif text_char_pointer == 0:
                            token_cased += token_char
...
def get_utt_obj(
    ...
    Function to create an Utterance object and add all necessary information to it except
        for timings of the segments / tokens according to the alignment - that will
        ...
        Within the Segment objects, there is a list tokens which contains Token objects for
    ...
    if hasattr(model, "tokenizer"):
        ...
        for segment in segments:
            ...
            for token_str_i, token_str in enumerate(token_strs):
                ...
                # add a Token object for blank between space-separated substrings
                if token_str_i < len(token_strs) - 1:
                    utt.segments_and_tokens[-1].tokens.append(
                        Token(
                            text=BLANK_TOKEN,
                            text_cased=BLANK_TOKEN,
        ...
        for i_segment, segment in enumerate(segments):
            ...
            for token_str_i, token_str in enumerate(token_strs):
                ...
                token_ids = get_char_tokens(token_str, model)

                for token_i, (token, token_id) in enumerate(zip(tokens, token_ids)):
                    # add the text tokens and the blanks in between them to token_ids_with_blanks
                    ...
                    # add blank tokens between tokens in a substring
                    if token_i < len(tokens) - 1:
                ...
                # add space token (and the blanks around it) unless this is the final substring in a segment
                if token_str_i < len(token_strs) - 1:
